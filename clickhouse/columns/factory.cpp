            ...
            if (settings.low_cardinality_as_wrapped_column) {
                ...
                switch (nested.code) {
                    ...
                    case Type::Nullable:
                    {
                        ...
                        {
                        case Type::String:
                            return std::make_shared<ColumnLowCardinalityT<ColumnString,true>>();
                        case Type::FixedString:
                            return std::make_shared<ColumnLowCardinalityT<ColumnFixedString,true>>(nullable_nested.elements.front().value);
                        default:
                            throw std::runtime_error("LowCardinality(Nullable(" + nullable_nested.name + ")) is not supported");
                        }
                    }
