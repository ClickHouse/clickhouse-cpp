...
public:
    ColumnNothing() : Column(Type::CreateNothing()), size_(0) {}

    explicit ColumnNothing(size_t n) : Column(Type::CreateNothing()), size_(n) {}
    ...
    ColumnRef Slice(size_t, size_t len) override { return std::make_shared<ColumnNothing>(len); }
...
public:
    ...
    void Save(CodedOutputStream*) override { throw std::runtime_error("method Save is not supported for Nothing column"); }
