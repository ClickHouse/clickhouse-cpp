...
void ItemView::ValidateData(Type::Code type, DataType data) {
    ...
    if (type >= sizeof(value_size_by_type) / sizeof(value_size_by_type[0]) || type < 0) {
        ...
        if (expected_size == ERR) {
            ...
            throw std::runtime_error("Value size mismatch for type " + std::to_string(static_cast<int>(type)) +
                                     " expected: " + std::to_string(expected_size) + ", got: " + std::to_string(data.size()));
        }
    }
}
