...
ColumnNullable::ColumnNullable(ColumnRef nested, ColumnRef nulls)
    : Column(Type::CreateNullable(nested->Type())), nested_(nested), nulls_(nulls->As<ColumnUInt8>()) {
...
void ColumnNullable::Swap(Column& other) {
    ...
    if (!nested_->Type()->IsEqual(col.nested_->Type())) throw std::runtime_error("Can't swap() Nullable columns of different types.");
...
ItemView ColumnNullable::GetItem(size_t index) const {
    if (IsNull(index)) return ItemView();

    return nested_->GetItem(index);
}

std::ostream& ColumnNullable::Dump(std::ostream& o, size_t index) const {
    if (IsNull(index)) {
        return o << "NULL";
    }
    Nested()->Dump(o, index);
