...
static TypeAst::Meta GetTypeMeta(const std::string_view& name) {
    if (name == "Array") {
...
TypeParser::TypeParser(const std::string_view& name)
    : cur_(name.data())
...
TypeParser::Token TypeParser::NextToken() {
    static const auto EMPTY = std::string_view{};
